...
# Part 3: Clip the image to fit within the CMYK gamut and compute Delta E
def clip_image_and_compute_delta_e():
    # Load the image and convert to LAB, then LCH
    image_path = 'newdataset/sample.jpg'  # Update with your image path
    image = cv2.imread(image_path)
    if image is None:
    ...
    # Calculate Delta E 2000 between original and clipped LAB values
    original_lab = image_lab.reshape((-1, 3))  # Flatten the original LAB image
    clipped_lab = clipped_lab_image.reshape((-1, 3))  # Flatten the clipped LAB image

    # Compute Delta E 2000
    delta_e_2000 = deltaE_ciede2000(original_lab, clipped_lab)
    
    # Print the overall average Delta E 2000 value
    avg_delta_e = np.mean(delta_e_2000)
    print(f"Average Delta E 2000 between original and clipped image: {avg_delta_e:.4f}")
...
plot_gamuts()
gamut_mapping()
clip_image_and_compute_delta_e()
